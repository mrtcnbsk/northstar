...
describe("XcodeBuildTool execute: timeout + failing kill", () => {
  // Pins the never-crash invariant on the timeout+kill-failure path: xcodebuild runs past the
  ...
  harness.effect("timeout + a FAILING kill still returns a structured ok:false result, never crashes", () =>
    Effect.gen(function* () {
      ...
      // Advance virtual time past the build timeout so the timeout branch (with its failing kill)
      // fires deterministically.
      yield* TestClock.adjust(`${BUILD_TIMEOUT_MS + 1000} millis`)

      // The fiber must complete with a value (not die): joining it does not raise.
      const result = yield* Fiber.join(fiber)
      const summary = JSON.parse(result.output)
      expect(summary.ok).toBe(false)
      // Timeout is surfaced as a build failure (nonzero exit + a terminated-after-timeout note).
      expect(summary.status).toBe("build_failed")
