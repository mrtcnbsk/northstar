...
export namespace ConsoleAssets {
  ...
  async function build(app: string, out: string) {
    ...
    const run = runBuild(app, out).catch((err) => {
      log.warn("failed to build Northstar Console assets", { err })
      return undefined
    })
    builds.set(app, run)
    return await run
  ...
  async function runBuild(app: string, out: string) {
    log.info("building Northstar Console assets", { app })
    ...
    if (code !== 0) throw new Error(`Northstar Console build failed with exit code ${code}: ${stderr || stdout}`)
    if (!(await ready(out, true))) throw new Error("Northstar Console build did not produce /console assets")
    return out
  }
