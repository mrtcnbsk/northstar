...
const fallbackWordmark = wordmark.map((line) => line.replaceAll("█", "#"))
const exit = [`  NORTHSTAR  `, `             `, `             `]

const modern = {
  tui: wordmark.map((line, index) => (index === 0 ? `🬺🬏 ${line}` : `   ${line}`)),
  plain: wordmark,
  exit,
}

const fallback = {
  tui: fallbackWordmark,
  plain: fallbackWordmark,
  exit,
}
...
export function session(
  ...
  return [``, `${logo[0]}${dim}${title}${normal}`, `${logo[1]}${dim}northstar -s ${id}${normal}`, logo[2]].join("\n")
}
