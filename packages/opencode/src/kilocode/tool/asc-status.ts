  ...
  Effect.succeed({
    ...
    execute: (params: Params, ctx: Tool.Context) =>
      Effect.gen(function* () {
        ...
        // kilocode_change - finding #2: `Effect.promise` assumes the promise never rejects, but a
        // corrupt/unreadable auth.json makes `loadAscCredential` reject (JSON.parse defect -> the
        // underlying runPromise rejects), which would otherwise DIE here as an unrecovered defect
        // and throw a raw error (risking a leaked auth-store path) instead of degrading to the
        // clean "unavailable" message below. `Effect.tryPromise` + `orElseSucceed` converts ANY
        // rejection into the same `undefined` the "no credential configured" case already handles.
