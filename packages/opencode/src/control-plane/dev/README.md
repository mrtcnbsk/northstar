...
In a separate terminal, run a separate Northstar server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/opencode/script/run-workspace-server
```

With the plugin install, you can now run Northstar and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
