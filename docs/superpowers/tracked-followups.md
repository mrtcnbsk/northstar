...
- **CLI version 7.4.5 → 0.1.0** (`packages/opencode/package.json`) — temiz ilk-public-release semver'i. **NOT: yayınlanan version `Script.version` (release tag / workflow `version` input) tarafından belirlenir** (root publish.ts:46 tüm package.json version'larını Script.version'a set eder) — 0.1.0 yalnız dev `--version` için; gerçek release version'ı KULLANICI release tag'iyle kontrol eder.
